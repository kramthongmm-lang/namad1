 ...
 ============================================================ */
  ...
  const getFreeCourts = () => {
    return courts.filter(
      c => c.Cort_Status === "Active" && !lockedCourtsRef.current.has(c.Cort)
    );
  };
...
const getPlayerRank = (playerName) => {
  ...
  const buildAutoMatch = () => {
    ...
    const scoreMode = getAutoScoreMode();
    // ============================
    // ? PHASE 0 (GLOBAL) ?
    // ============================
    const same4 = findFourSameScore(autoCandidates);
    if (same4) {
      const [a, b, c, d] = same4;

      // ?????????????????
      if (
        violatesHardLimit(a, b, c, d) ||
        partneredLastMatch(a, b) ||
        partneredLastMatch(c, d) ||
        metLastMatch(a, c) ||
        metLastMatch(a, d) ||
        metLastMatch(b, c) ||
        metLastMatch(b, d)
        ...
        return buildResult(
          players.find(p => p.Player_Name === a),
          players.find(p => p.Player_Name === b),
          players.find(p => p.Player_Name === c),
          players.find(p => p.Player_Name === d),
          [],
          [],
          `AUTO: Phase 0 (Same Score All 4)`
        );
      }
    }
 ...
 const phases = [
  { name: "Phase 0 (Same Score All 4)", mode: "sameScoreAll", rankGap: 0 },
  { name: "Phase 0.5 (Same Score 3 of 4)", mode: "sameScoreThree", rankGap: 0 },
  { name: "Phase 1 (Same Rank)", rankGap: 0 },
  { name: "Phase 2 (Allow Rank ±1)", rankGap: 1 },
  { name: "Phase 3 (Allow Rank ±2)", rankGap: 2 },
  { name: "Phase 4 (Free Rank)", rankGap: Infinity },
];
  ...
  const partnerOK = canBePartnerByRankGap(
  P1,
  p2Name,
  phase.rankGap
);
if (!partnerOK) continue;
       ...
       const oppOK = canBePartnerByRankGap(
  p3,
  p4,
  phase.rankGap
);
if (!oppOK) continue;
            ...
            // ?? ??????????????????
            if (!enforcePairForOpposite(p3, p4)) continue;

            // ?? partner / opponent = 2
            if (violatesHardLimit(P1, p2Name, p3, p4)) continue;
            ...
            if (scoreMode === "sameScore") {
              // ?? ?????????????
              if (!sameScoreAllFour(P1, p2Name, p3, p4)) continue;
              scoreInfo = teamScoreInfo(P1, p2Name, p3, p4);
            ...
            return buildResult(
              ...
              [],
              `AUTO: ${phase.name} | ` +
              `TeamA = ${scoreInfo?.teamA ?? "-"} vs ` +
              `TeamB = ${scoreInfo?.teamB ?? "-"} | ` +
              `diff = ${scoreInfo?.diff ?? 0}`
            );
          }
        }
      }
