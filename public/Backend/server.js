...
const Match = () => {
  ...
  const [nextMatch, setNextMatch] = useState(null); // ????????????? (Queue)
  // Queue ???????
...
const [rankExposure, setRankExposure] = useState({});
  ...
  const autoStartingCourtRef = useRef(null); // ? ???????????
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    console.log("?? Reset locked courts (backend updated)");
  }, [courts]);
                ...
                return sorted.map((p, i) => {
                  ...
                  return (
                    <div key={i} className={`rank-item ${classColor}`}>
                      ...
                      <span className="rank-score">{p.count} ???</span>
                    </div>
                  );
                });
              })()}
      ...
      <div className="match-grid">
        ...
        <div className="">
          ...
          <div style={{ fontSize: 13, opacity: 0.8, marginTop: 6 }}>
            ...
            onClick={() => {
              setAutoQueueOn(v => !v);
              autoQueuedRef.current = false; // reset ???????
            }}
          >
            {autoQueueOn ? "? ????????????????????" : "? ?????????????????????"}
          </button>
          ...
          {showPreview && previewMatches && previewMatches.length > 0 && (
            <div className="preview-panel-compact">
              ...
              <h3>?????? / ????? Match ??????? ({previewMatches.length})</h3>
        ...
        {/* ? Rank Exposure — ????????? P1 */}
              ...
              <div className="action-buttons">
                ...
                <button
                  className="btn green"
                  onClick={() => {
                    confirmCreateMatch(previewMatches[0]);   // ? ??? match ???
                    setPreviewMatch(null);
                    setShowPreview(false);
                  }}
                >
                ...
                <button
                  className="btn blue"
                  onClick={() => {
                    autoQueuedRef.current = true; // ??? auto ???
                    autoQueueNext();
                  }}
                >
                  ? ????????
          ...
          <RandomPanel
            ...
            group={group}
            lastEnd={lastEnd}   // ? ?????????
            now={now}           // ? ???????????????                     // ? ??? group ??? RandomPanel
            openPreview={openPreview}
            openPreviewMulti={openPreviewMulti}
            updatePlayerFlagInDB={updatePlayerFlagInDB}   // ? ?????????????? Flag_Player DB
